...
public class CrdtFs extends FuseStubFS {
	...
	public CrdtFs() {
		...
		/*Bucket<String> bucket = Bucket.create("fsBucket");
		AntidoteStaticTransaction tx = antidote.createStaticTransaction();
		rootDirectory = bucket.map_aw("root");
		rootDirectory.map_aw("dirA").register("file1", ValueCoder.bytestringEncoder).set(tx,
	...
	System.out.println("getattr path: " + path);
		...
		int resp = 0;
		switch (StringUtils.countMatches(path, "/")) {
					...
					for (MapKey<String> key : res.mapKeySet())
						if (key.getKey().equals(path.substring(1))) {
							if (key.getType().equals(CRDT_type.AWMAP)) {
								...
								stat.st_nlink.set(1);
								stat.st_size.set(res.get(key, ResponseDecoder.register()).get().getBytes().length);
							}
							resp = 0;
							break;
						}
			...
			case 2:
				// dirA folder
				if (path.substring(path.lastIndexOf("/")).equals("file1")) {
					stat.st_mode.set(FileStat.S_IFREG | 0444);
					stat.st_nlink.set(1);
					stat.st_size.set(100); // XXX
//					MapRef.MapReadResult<String> res = rootFolder.read(antidote.noTransaction());
//					res.map_aw("dirA", ValueCoder.bytestringEncoder).register("file1", ValueCoder.bytestringEncoder).getValue();
				} else
					resp = -ErrorCodes.ENOENT();
				break;
	...
	public int readdir(String path, Pointer buf, FuseFillDir filter, @off_t long offset, FuseFileInfo fi) {
		...
		if ("/".equals(path)) {
			...
			for (String key : res.keySet())
				filter.apply(buf, key, null, 0);
		} else if ("/dirA".equals(path)){
			filter.apply(buf, "file1", null, 0);
		} else 
	...
	*/
	...
	public int create(String path, @mode_t long mode, FuseFileInfo fi) {
		...
		FsElement parent = getParentPath(path);
	...
	private FsElement getParentPath(String path) {
		return rootDirectory.find(path.substring(0, path.lastIndexOf("/")));
	}

	private FsElement getPath(String path) {
	...
	public int rename(String path, String newName) {
		...
		FsElement newParent = getParentPath(newName);
		if (newParent == null) {
			return -ErrorCodes.ENOENT();
		}
